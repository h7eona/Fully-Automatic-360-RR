...
class LinearProjection(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0., bias=True, qflip=False):
        super().__init__()
        inner_dim = dim_head *  heads
        self.heads = heads
...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size, num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., qflip=False):
        ...
        if token_projection =='conv':
            ...
            self.qkv = LinearProjection(dim,num_heads,dim//num_heads,bias=qkv_bias,qflip=qflip)
        else:
...
class LeFF(nn.Module):
    def __init__(self, input_resolution=(256,512), dim=32, hidden_dim=128, act_layer=nn.GELU,drop = 0., use_eca=False):
        super().__init__()
    ...
    def forward(self, x):
        # bs x hw x c
        bs, hw, c = x.size()
        hh = self.input_resolution[0]
        ww = self.input_resolution[1]
...
def window_partition(x, win_size, dilation_rate=1):
    ...
    if dilation_rate !=1:
        ...
        x = F.unfold(x, kernel_size=win_size[0],dilation=dilation_rate,padding=4*(dilation_rate-1),stride=win_size[0]) # B, C*Wh*Ww, H/Wh*W/Ww
        windows = x.permute(0,2,1).contiguous().view(-1, C, win_size[0], win_size[1]) # B' ,C ,Wh ,Ww
        windows = windows.permute(0,2,3,1).contiguous() # B' ,Wh ,Ww ,C
    else:
        width_window_size = win_size[1]
        height_window_size = win_size[0]
        x = x.view(B, H // height_window_size, height_window_size, W // width_window_size, width_window_size, C)
        # x = x.view(B, H // win_size, win_size, W // win_size, win_size, C)
        windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, height_window_size, width_window_size, C) # B' ,Wh ,Ww ,C
...
def window_reverse(windows, win_size, H, W, dilation_rate=1):
    ...
    width_window_size = win_size[1]
    height_window_size = win_size[0]
    B = int(windows.shape[0] / (H * W / height_window_size / width_window_size))
    # B = int(windows.shape[0] / (H * W / win_size / win_size))
    x = windows.view(B, H // height_window_size, W // width_window_size, height_window_size, width_window_size, -1)
    # x = windows.view(B, H // win_size, W // win_size, win_size, win_size, -1)
    if dilation_rate !=1:
        x = windows.permute(0,5,3,4,1,2).contiguous() # B, C*Wh*Ww, H/Wh*W/Ww
        x = F.fold(x, (H, W), kernel_size=win_size[0], dilation=dilation_rate, padding=4*(dilation_rate-1),stride=win_size[0])
    else:
...
class Downsample(nn.Module):
    def __init__(self, in_channel, out_channel, input_resolution):
        super(Downsample, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d((in_channel+3), out_channel, kernel_size=4, stride=2, padding=1),
        )
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.input_resolution = input_resolution
...
class OutputProj(nn.Module):
    def __init__(self, in_channel=64, out_channel=3, kernel_size=3, stride=1, norm_layer=None,act_layer=None, input_resolution=(256,512)):
        super().__init__()
        self.proj = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=kernel_size//2),
        )
...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 ...
                 modulator=False,cross_modulator=False, qflip=False):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        ...
        self.height_window_size = 1
        self.width_window_size = self.input_resolution[1]
        ...
        if modulator:
            self.modulator = nn.Embedding(self.height_window_size*self.width_window_size, dim) # modulator
        else:
            self.modulator = None
        ...
        self.attn = WindowAttention(
            dim, win_size=(self.height_window_size, self.width_window_size), num_heads=num_heads,
            # dim, win_size=to_2tuple(self.win_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            token_projection=token_projection, qflip=qflip)
        ...
        if token_mlp in ['ffn','mlp']:
            ...
            self.mlp =  LeFF(self.input_resolution, dim, mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, mask=None):
        ...
        if mask != None:
            ...
            input_mask_windows = window_partition(input_mask, (self.height_window_size, self.width_window_size)) # nW, win_size, win_size, 1
            attn_mask = input_mask_windows.view(-1, self.height_window_size * self.width_window_size) # nW, win_size*win_size
        ...
        #             shift_mask[:, h, w, :] = cnt
        #             cnt += 1
        #     shift_mask_windows = window_partition(shift_mask, (self.height_window_size, self.width_window_size))  # nW, win_size, win_size, 1
        #     shift_mask_windows = shift_mask_windows.view(-1, self.height_window_size * self.width_window_size) # nW, win_size*win_size
        ...
        ## SHIFT MASK FOR HORZWIN
        if self.shift_size > 0 and self.height_window_size == 2:
            shift_mask = torch.zeros((1, H, W, 1)).type_as(x)
            shift_mask_windows = window_partition(shift_mask, (self.height_window_size, self.width_window_size))  # nW, win_size, win_size, 1
            shift_mask_windows = shift_mask_windows.view(-1, self.height_window_size * self.width_window_size)
            nW, L = shift_mask_windows.shape
            shift_attn_mask = torch.zeros((nW, L, L)).type_as(x)

            shift_attn_mask[:, L//2:, :L//2] = float(-100.0)
            shift_attn_mask[:, :L//2, L//2:] = float(-100.0)
            shift_attn_mask = shift_attn_mask.masked_fill(shift_attn_mask == 0, float(0.0))
            # shift_mask_windows = shift_mask_windows.view(-1, self.height_window_size * self.width_window_size) # nW, win_size*win_size
            # shift_attn_mask = shift_mask_windows.unsqueeze(1) - shift_mask_windows.unsqueeze(2) # nW, win_size*win_size, win_size*win_size
            # #### print(shift_attn_mask.shape) = # nW, win_size*win_size, win_size*win_size
            # shift_attn_mask = shift_attn_mask.masked_fill(shift_attn_mask != 0, float(-100.0)).masked_fill(shift_attn_mask == 0, float(0.0))
        ...
        # partition windows
        x_windows = window_partition(shifted_x, (self.height_window_size, self.width_window_size))  # nW*B, win_size, win_size, C  N*C->C

        x_windows = x_windows.view(-1, self.width_window_size * self.height_window_size, C)  # nW*B, win_size*win_size, C
        ...
        attn_windows, attn = self.attn(wmsa_in, mask=attn_mask)  # nW*B, win_size*win_size, C

        # merge windows
        attn_windows = attn_windows.view(-1, self.height_window_size, self.width_window_size, C)
        # attn_windows = attn_windows.view(-1, self.win_size, self.win_size, C)
        shifted_x = window_reverse(attn_windows, (self.height_window_size, self.width_window_size), H, W)  # B H' W' C
...
class BasicUformerLayer(nn.Module):
        ...
        if shift_flag:
            self.blocks = nn.ModuleList([
                LeWinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                    ...
                                    modulator=modulator,cross_modulator=cross_modulator, qflip=qflip)
    ...
    def forward(self, x, mask=None):
        ...
        # Block Mean
        # attn_mean = torch.mean(attns, dim=0)
        return x, attns
...
class LaplacianPyramid(nn.Module):
    def __init__(self, dim=3):
        ...
        laplacian_kernel = np.repeat(laplacian_kernel[None, None, :, :], 3, 0)
        self.kernel = torch.nn.Parameter(torch.FloatTensor(laplacian_kernel).cuda())
    ...
    def forward(self, x):
        x0 = F.interpolate(x, scale_factor=0.0625, mode='bilinear')
        x1 = F.interpolate(x, scale_factor=0.125, mode='bilinear')
        ...
        lap_3 = F.conv2d(x3, self.kernel, groups=self.channel_dim, padding=1, stride=1, dilation=1)
        lap_4 = F.conv2d(x, self.kernel, groups=self.channel_dim, padding=1, stride=1, dilation=1)

        return lap_0, lap_1, lap_2, lap_3, lap_4
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 ...
                 cross_modulator=False, qflip=False, **kwargs):
        super().__init__()
        ...
        # Input/Output
        self.lap_pyramid = LaplacianPyramid(dim=3)
        self.input_proj = InputProj(in_channel=dd_in, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        self.output_proj = OutputProj(in_channel=2*embed_dim, out_channel=in_chans, kernel_size=3, stride=1, input_resolution=self.reso)
        
        # Encoder
        self.encoderlayer_0 = BasicUformerLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(self.reso[0],
                                                self.reso[1]),
                            depth=depths[0],
                            ...
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag, qflip=qflip)
        self.dowsample_0 = dowsample(embed_dim, embed_dim*2, input_resolution=(self.reso[0], self.reso[1]))
        self.encoderlayer_1 = BasicUformerLayer(dim=embed_dim*2,
                            output_dim=embed_dim*2,
                            input_resolution=(self.reso[0] // 2,
                                                self.reso[1] // 2),
                            depth=depths[1],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag, qflip=qflip)
        self.dowsample_1 = dowsample(embed_dim*2, embed_dim*4, input_resolution=(self.reso[0] // 2 , self.reso[1] // 2))
        self.encoderlayer_2 = BasicUformerLayer(dim=embed_dim*4,
                            output_dim=embed_dim*4,
                            input_resolution=(self.reso[0] // (2 ** 2),
                                                self.reso[1] // (2 ** 2)),
                            depth=depths[2],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag, qflip=qflip)
        self.dowsample_2 = dowsample(embed_dim*4, embed_dim*8, input_resolution=(self.reso[0] // (2 ** 2), self.reso[1] // (2 ** 2)))
        self.encoderlayer_3 = BasicUformerLayer(dim=embed_dim*8,
                            output_dim=embed_dim*8,
                            input_resolution=(self.reso[0] // (2 ** 3),
                                                self.reso[1] // (2 ** 3)),
                            depth=depths[3],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag, qflip=qflip)
        self.dowsample_3 = dowsample(embed_dim*8, embed_dim*16, input_resolution=(self.reso[0] // (2 ** 3), self.reso[1] // (2 ** 3)))

        # Bottleneck
        self.conv = BasicUformerLayer(dim=embed_dim*16,
                            output_dim=embed_dim*16,
                            input_resolution=(self.reso[0] // (2 ** 4),
                                                self.reso[1] // (2 ** 4)),
                            depth=depths[4],
        ...
        # Decoder
        self.upsample_0 = upsample(embed_dim*16, embed_dim*8, input_resolution=(self.reso[0] // (2 ** 4), self.reso[1] // (2 ** 4)))
        self.decoderlayer_0 = BasicUformerLayer(dim=embed_dim*16,
                            ...
                            modulator=modulator,cross_modulator=cross_modulator, qflip=qflip)
        self.upsample_1 = upsample(embed_dim*16, embed_dim*4, input_resolution=(self.reso[0] // (2 ** 3), self.reso[1] // (2 ** 3)))
        self.decoderlayer_1 = BasicUformerLayer(dim=embed_dim*8,
                            ...
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            modulator=modulator,cross_modulator=cross_modulator, qflip=qflip)
        self.upsample_2 = upsample(embed_dim*8, embed_dim*2, input_resolution=(self.reso[0] // (2 ** 2), self.reso[1] // (2 ** 2)))
        self.decoderlayer_2 = BasicUformerLayer(dim=embed_dim*4,
                            ...
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            modulator=modulator,cross_modulator=cross_modulator, qflip=qflip)
        self.upsample_3 = upsample(embed_dim*4, embed_dim, input_resolution=(self.reso[0] // 2, self.reso[1] // 2))
    ...
    def forward(self, x, mask=None):
        # Input Projection
        lap_0, lap_1, lap_2, lap_3, lap_4 = self.lap_pyramid(x)
        y = self.input_proj(x, lap_4)
        y = self.pos_drop(y)
        #Encoder
        conv0, attn0 = self.encoderlayer_0(y,mask=mask)
        pool0 = self.dowsample_0(conv0, lap_4)
        conv1, attn1 = self.encoderlayer_1(pool0,mask=mask)
        pool1 = self.dowsample_1(conv1, lap_3)
        conv2, attn2 = self.encoderlayer_2(pool1,mask=mask)
        pool2 = self.dowsample_2(conv2, lap_2)
        conv3, attn3 = self.encoderlayer_3(pool2,mask=mask)
        pool3 = self.dowsample_3(conv3, lap_1)

        # Bottleneck
        conv4, attn4 = self.conv(pool3, mask=mask)

        #Decoder
        up0 = self.upsample_0(conv4)
        deconv0 = torch.cat([up0,conv3],-1)
        deconv0, attn5 = self.decoderlayer_0(deconv0,mask=mask)
        
        up1 = self.upsample_1(deconv0)
        deconv1 = torch.cat([up1,conv2],-1)
        deconv1, attn6 = self.decoderlayer_1(deconv1,mask=mask)

        up2 = self.upsample_2(deconv1)
        deconv2 = torch.cat([up2,conv1],-1)
        deconv2, attn7 = self.decoderlayer_2(deconv2,mask=mask)

        up3 = self.upsample_3(deconv2)
        deconv3 = torch.cat([up3,conv0],-1)
        deconv3, attn8 = self.decoderlayer_3(deconv3,mask=mask)

        # Output Projection
        y = self.output_proj(deconv3)
        # return x + y if self.dd_in ==3 else y, [attn0, attn1, attn2, attn3, attn4, attn5, attn6, attn7, attn8]
        return x + y, [attn0, attn1, attn2, attn3, attn4, attn5, attn6, attn7, attn8]
